...
const SearchResultsPage = () => {
  ...
  const subcategoriesArray = subcategoriesParam
    ? subcategoriesParam.split(" ").map(decodeURIComponent)
    : [];
  ...
  return (
    <>
      <PageHelmet
        title={`${query} Search Results | Electrozone`}
        description="Find the electronics you need with our powerful search tool, showcasing all available products at Electrozone."
      />
          ...
          <div className="-mt-4 flex flex-row items-start space-x-2">
            ...
            >
                  ...
                  <FormControlLabel
                    ...
                    sx={{
                      "& .MuiFormControlLabel-label": {
                        // Targeting the label of the FormControlLabel
              ...
              <div className="mt-2 w-[95%]">
                <h4 className="mb-3 mt-2 text-lg font-semibold">Price Range</h4>
                <Slider
                  getAriaLabel={() => "Price range"}
                  value={priceRange}
                  onChange={handlePriceChange}
            ...
            <div className="flex grow flex-wrap sm:mt-4">
              ...
              <ProductList productsData={productsData} ref={observerTarget} />
            </div>
          </div>
        </Suspense>
      </div>
...
export async function loader({ request }: any) {
  ...
  const { products, availableBrands, priceRange, availableSubcategories, productQuantity } =
    await searchProducts(query!, sort, 0, initProdCount, filters);
