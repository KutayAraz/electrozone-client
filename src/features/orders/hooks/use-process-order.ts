...
import { selectCheckoutIntent } from "@/stores/slices/user-slice";
...
export const useProcessOrder = () => {
  ...
  const userIntent: CheckoutType = useAppSelector(selectCheckoutIntent);
  const [processOrder, { isLoading }] = useProcessOrderMutation();

  const placeOrder = async (checkoutSnapshotId: string, refetchCartFunction: any) => {
    try {
      ...
      if (error.status === 400 || error.status === 409) {
        ...
        if (
          ...
          return refetchCartFunction(userIntent);
        }
      } else {
